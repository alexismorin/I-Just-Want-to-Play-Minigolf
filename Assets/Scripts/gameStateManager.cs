...
public class gameStateManager : MonoBehaviour
{
    ...
    {
        if (loadedSettings[0] == "1")
        { // use the seed provided on line 2 of settings.ini
            Random.InitState(int.Parse(loadedSettings[1]));
        }
        else
        { // random seed - do this if you want to play random matches every time
    ...
    void BuildMap()
    { // build golf green, square of size is line 3 of settings.ini
        ...
        startTile = tiles[Mathf.RoundToInt(tiles.Length / 2f)];
